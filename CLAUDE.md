...
| Dev (both apps) | `bun dev`           | `turbo run dev` — server :3000 + client :5173 in parallel |
| Build all       | `bun run build`     | `turbo run build` — server bundle + client static bundle  |
| Type-check all  | `bun run typecheck` | `turbo run typecheck` — tsc + svelte-check          |
| Run tests       | `bun run test`      | `turbo run test` — Vitest in `apps/server`             |
...
```txt
browser (apps/client, Svelte 5)          external clients (curl, SDKs)
  │  POST /v1/ai/chat                   │  POST /v1/chat/completions
  │  (AI SDK UI message stream)         │  (OpenAI-compatible SSE/JSON)
  ▼                                     ▼
apps/server/src/index.ts   (logger → cors → requestId → routes → onError/notFound)
...
- **`chat-service` as canonical internal interface:** both route adapters call `getChatService().generate(...)` / `.stream(...)`. Routes are protocol shims only — no model/provider logic leaks past `apps/server/src/services/chat-service.ts`.
- **Vite proxy** in `apps/client/vite.config.ts` forwards `/v1/*` to `http://localhost:3000`. No CORS in dev. The existing CORS middleware on the API stays in place for cross-origin prod deployments.
- **Entrypoint (`apps/server/src/index.ts`):** Builds the app via `createRouter()` so `Variables` is typed, applies global middleware in order (`logger`, `cors`, `requestId`), exposes `GET /healthz`, mounts route groups with `app.route()`, then calls `registerErrorHandler(app)`. Exports named `app` (for tests) plus a default `{ port, fetch }` object that Bun auto-serves when the file is the CLI entrypoint.
- **Router factory (`apps/server/src/utils/create-router.ts`):** `createRouter()` returns `new Hono<{ Variables: { requestId: string } }>()`. Every sub-router uses it so `c.get('requestId')` is typed everywhere.
- **Middleware (`apps/server/src/middleware/`):** `requestId` reads `x-request-id` (or generates a UUID), stores it in context, mirrors it back in the response header. The Vite proxy preserves this header end-to-end.
- **Config (`apps/server/src/config/env.ts`):** Single Zod schema parses `process.env` at module load. `LLM_API_KEY`, `LLM_BASE_URL`, `DEFAULT_MODEL`, `PORT`, `NODE_ENV`. Failures throw one aggregated error naming every missing/invalid field.
- **Service layer (`apps/server/src/services/chat-service.ts`):** `createChatService(agent)` returns `{ generate, stream }`. A lazy `getChatService()` returns the singleton wired to the default Mastra agent. Tests mock this module.
- **Mastra layer (`apps/server/src/mastra/`):** `default-agent.ts` reads from `env` and builds one `Agent` (OpenAI-compatible model config). `index.ts` registers it on a `Mastra` instance and exposes `getDefaultAgent()`.
- **Chat route (`apps/server/src/routes/chat.ts`):** Validates the OpenAI-shaped request with `zValidator`, delegates to `getChatService()`, and translates outputs into OpenAI `chat.completion` / `chat.completion.chunk` SSE frames via pure helpers in `chat-mapping.ts`. Streaming errors emit a terminal `{ error: {...} }` SSE frame + `[DONE]` because `onError` can't reach an already-open stream.
- **AI chat route (`apps/server/src/routes/ai-chat.ts`):** Validates the AI SDK UI message request with `zValidator` over `aiChatRequestSchema`, flattens `parts[]` into plain text `ChatMessage[]`, delegates to `getChatService().stream()`, and writes `text-start` → `text-delta` → `text-end` frames via the AI SDK's `createUIMessageStream` / `createUIMessageStreamResponse`. Stream errors are surfaced as the protocol's own `error` frame (string) via `onError`.
- **Error handling (`apps/server/src/utils/error-handler.ts`):** `registerErrorHandler(app)` attaches `onError` and `notFound`. Errors map to `{ error: { code, message, requestId } }`. `HTTPException` → its own status, `ZodError` → 400 `validation_error`, everything else → 500 `internal_error` with a generic message (never leaks `err.message`).
- **Web app (`apps/client/src/App.svelte`):** Svelte 5 runes (`$state`). Uses the AI SDK (`@ai-sdk/svelte`'s `Chat` + `DefaultChatTransport` from `ai`) to call `/v1/ai/chat`. The SDK handles transport, streaming parse, and message state; the component just renders `chat.messages` and forwards input via `chat.sendMessage`.
...
`.env` lives at **`apps/server/.env`**, not the repo root. Per Turborepo best practice, a root `.env` implicitly couples every package; keeping it inside the app that consumes it makes the dependency explicit. Bun auto-loads `.env` from the cwd at startup, and `turbo run dev` invokes the api script with `cwd=apps/server`.
...
`apps/server/src/config/env.ts` is the single source of truth — a Zod schema over `process.env`, parsed at module load. On failure it throws one aggregated error naming every missing/invalid field. Everything else imports typed values from there; no module reads `process.env` directly.
...
Because the agent uses Mastra's `OpenAICompatibleConfig` (just `id`, `url`, `apiKey`), any OpenAI-compatible chat-completions server works without a code change — only `apps/server/.env` changes.
...
- **`jsx: "react-jsx"` with `jsxImportSource: "hono/jsx"`** (api only) — Hono JSX is pre-wired. `.tsx` files in `apps/server` render through Hono's JSX runtime, not React.
- **`strict: true`** — no implicit `any`, strict null checks, etc.
- **`types: ["bun"]`** in `apps/server` — `@types/bun` supplies Bun globals (`Bun`, `import.meta.main`) plus Node-compat types (`process`, `crypto`, etc.). Do not add `@types/node` — it conflicts with the Bun types.
- **`apps/client` uses Svelte's tsconfig** — extends `@tsconfig/svelte`, types include `svelte` and `vite/client`. No Bun types here; the web app runs in the browser.
